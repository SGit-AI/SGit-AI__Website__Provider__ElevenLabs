...
**What was done, in two stages.** It was first published as a plan with its numbers badged; then, on 8 September 2026, **the project lead supplied a key and it was run** — the first execution of that path by anyone. [What it found is on /video/](/video/): two refusals, one prediction wrong, one failure nobody predicted, and the measurement that closes the drift question {{claim:alignment-exact}}. The video and its materials are in an encrypted vault rather than in this repository.

The three reasons it was not run *before* that key arrived still stand, and the first one is why the exception is dated rather than quiet:

1. **No key, and that is a rule rather than an inconvenience.** The site is built with no key present, and every {{badge:verified}} chip came from a human's own browser rather than from this repository. **Asking** for a key would have traded that property for a two-minute film; being **given** one, for one run, with the exception recorded and dated, does not — and the key was never written to a file or committed, which the required secret scan enforces.
